...
async function connectFactory() {
  ...
  factoryContract = new ethers.Contract(factoryContractAddress, factoryContractAbi, ethersProvider.getSigner());
...
async function connectNFTContract(NFTContractAddress) {
  try {
    NFTContract = new ethers.Contract(NFTContractAddress, NFTContractAbi, ethersProvider.getSigner());
  } catch (error) {
    console.error(error);
  }
  return NFTContract;
...
function updateWalletAddress() {
  const walletAddressElement = document.getElementById("connect-wallet-button");
  if (connectedWalletAddress) {
    walletAddressElement.innerText = `Connected Wallet: ${connectedWalletAddress}`;
    document.getElementById("connect-wallet-button").style.backgroundColor = "green";
  } else {
    walletAddressElement.innerText = "";
    document.getElementById("connect-wallet-button").style.backgroundColor = "";
  }
...
async function getContractsByOwner(owner) {
  try {
    ...
    // Get the contracts by owner
    // const owner = ethersProvider.selectedAddress;
    const contracts = await factoryContract.getContractsByOwner(owner);

    // Clear the existing collection blocks
    const collectionContainer = document.querySelector(".collection-container");
    collectionContainer.innerHTML = "";

    // Loop through the contracts and display them as collection blocks
...
async function createCollection() {
  // Prompt the user to enter the name and symbol for the collection
  const name = prompt("Enter the name for the collection:");
  const symbol = prompt("Enter the symbol for the collection:");

  // Call the createContract function from the factory contract
  try {


    // Call the createContract function
    const tx = await factoryContract.createContract(name, symbol);

    // Wait for the transaction to be mined
    await tx.wait();

    // Refresh the collection blocks
    getContractsByOwner();
  } catch (error) {
    console.error(error);
  }
...
addCollectionButton.addEventListener("click", createCollection);
